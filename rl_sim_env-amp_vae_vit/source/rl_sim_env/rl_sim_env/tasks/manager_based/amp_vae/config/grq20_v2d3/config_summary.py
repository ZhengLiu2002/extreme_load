...
class ConfigSummary:
    ...
    class env:
        ...
        num_actions = 12
        # base_ang_vel*3 + projected_gravity*3 + joint_pos*12 + joint_vel*12 + joint_torques*12 + actions*12 + commands*3
        num_actor_obs = 57
        # base_lin_vel*3 + base_ang_vel*3 + projected_gravity*3 + commands*3 + joint_pos*12 + joint_vel*12 + actions*12 + height_scan*187 + random_com*3 + random_mass*1
        num_critic_obs = 239
        num_amp_obs = 39  # joint_pos*12 + foot_pos*12 + base_lin_vel*2 + base_ang_vel_yaw*1 + joint_vel*12
        num_vae_obs = 57  # matches actor_obs length
